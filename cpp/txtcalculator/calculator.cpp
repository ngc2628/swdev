...
static int esplitanum(
  Asciistr str,Asciistr *strmant,unsigned char *sgnmant,
  Asciistr *strexp,unsigned char *sgnexp) {
...
unsigned int Calculator::doFmt(unsigned int numfmt) {
  ...
  if (numfmt==0 || m_numfmt==numfmt)
    return m_numfmt;
  int actfmt=(int)m_numfmt;
  m_numfmt=numfmt;
  int ok=actfmt,ll=m_input.len(),i10=0;
  Asciistr buf,errmsg("err : cannot convert ");
  if (ll>0) {
...
int Calculator::chgSgn() {
  ...
  unsigned char sgn=0,sgnmant=0,sgnexp=0;
  Asciistr strmant,strexp;
  int idx=esplitanum(m_input,&strmant,&sgnmant,&strexp,&sgnexp);
  if (idx>=0) {
...
static int xappendNumber(int fmt,Asciistr num,Asciistr inp,Asciistr *xinp) {
