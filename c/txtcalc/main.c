...
int ltok=1000;
int maxlen=1024;
const char *cmdopt[ncmdopt]={"chk","e2s","s2e","mod","fmt","quit","exit"};
...
int docalc(char *cmd[ntok],int ll) {
  ...
  if (strcmp(cmd[0],cmdopt[0])==0) {
    ...
    if (strlen(cmd[2])>0)
      fmt=atoi(cmd[2]);
    ret=mk_txtchknum(cmd[1],fmt,&i10);
    printf ("%s\n",ret<0 ? "nan" : (i10==1 ? "int" : "float"));
  }
  else if (strcmp(cmd[0],cmdopt[1])==0) {
    ret=mk_txtexp2simple(cmd[1],ll-1);
    printf ("%s\n",cmd[1]);
  }
  else if (strcmp(cmd[0],cmdopt[2])==0) {
    ret=mk_txtsimple2exp(cmd[1],ll-1);
    printf ("%s\n",cmd[1]);
  }
    ...
    ret=mk_txtmod(cmd[1],cmd[2],div,carry,ll-1);
    printf ("%s [%s]\n",carry,div);
    free(carry);
    free(div);
  }
    ...
    ret=mk_txtconvfmt(cmd[1],fmt1,fmt2,ll-1);
    printf ("%s\n",cmd[1]);
  }
  else {
    char *res=(char*)malloc(maxlen);
    memset(&res[0],0,maxlen);
    ret=mk_txtcalc((unsigned char)cmd[1][0],cmd[0],cmd[2],res,ll-1);
    int chkfmt=0;
    if (strlen(cmd[3])>0) {
      if (mk_txtchknum(cmd[3],10)==10) {
        mk_txtround(res,ltok,atoi(cmd[3]));
        if (strlen(cmd[4])>0)
          chkfmt=4;
      }
      else
    ...
    if (chkfmt>0) {
      if (strcmp(cmd[chkfmt],"bin")==0)
        mk_txtconvfmt(res,10,2,ll-1);
      else if (strcmp(cmd[chkfmt],"oct")==0)
        mk_txtconvfmt(res,10,8,ll-1);
      else if (strcmp(cmd[chkfmt],"hex")==0)
        mk_txtconvfmt(res,10,16,ll-1);
      else if (strcmp(cmd[chkfmt],"exp")==0) {
        mk_txtsimple2exp(res,ll-1);
      }
    }
    printf ("%s\n",res);
  }
