...
z=[5000.,5000.,5000.,5000.,6000.,6000.,6000.,6000.,7000.,7000.,7000.,7000.,...
   8000.,8000.,8000.,8000.,7200.,7200.,7200.,7200];
...
for j=1:5
    ...
    [fail,nX, lamdaX, cX] = Oe02be(4,s(k), x(k), w, smooth);
    if (length(fail)>0)
      printf ("%d, j=%d %s",__LINE__,j,fail);
    end
    [fail,nY, lamdaY, cY] = Oe02be(4, s(k), y(k), w, smooth);
    if (length(fail)>0)
      printf ("%d, j=%d %s",__LINE__,j,fail);
    end
    [fail,nTh, lamdaTh, cTh] = Oe02be(4, s(k), z(k), w, smooth);
    if (length(fail)>0)
      printf ("%d, j=%d %s",__LINE__,j,fail);
    end
    [fail,nF, lamdaF, cF] = Oe02be(4, s(k), F(k), w, smooth);
    if (length(fail)>0)
      printf ("%d, j=%d %s",__LINE__,j,fail);
    end
    ...
    figure(1);
    hold on;
    %plot3(spX(pos+(1:20)),spY(pos+(1:20)),spTh(pos+(1:20)),'r-');
    figure(2);
    hold on;
    % plot splined function
    plot3(sps(pos+(1:20)),spt(pos+(1:20)),spF(pos+(1:20)),'r-');
    pos = pos+20;
    ...
    if(j<5)
        ...
        figure(1);
        hold on;
        plot3(spX(pos+(1:20)),spY(pos+(1:20)),spTh(pos+(1:20)),'k-');
        figure(2);
        hold on;
        % plot splined function
        plot3(sps(pos+(1:20)),spt(pos+(1:20)),spF(pos+(1:20)),'k-');
        pos = pos+20;
    end
end
