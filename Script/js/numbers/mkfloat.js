...
mkfloat.exp2simple=function(num) {
  ...
  for (ii=idxsgn;ii<ll;ii++) {
    if (str.charCodeAt(ii)==mkuni.asciidec) {
  ...
  while (frac.length>0 && frac[frac.length-1]==mkuni.asciizero)
    frac.pop();
  if (frac.length>0)
    ans.push(mkuni.str4code(mkuni.asciidec));
...
mkfloat.simple2exp=function(num) {
  ...
  if (intg.length>0)
    intg.splice(1,0,mkuni.asciidec);
  else if (frac.length>0)
    frac.splice(1,0,mkuni.asciidec);
  else
    return mkuni.str4code(mkuni.asciizero);
...
mkfloat.cut2int=function(num) {
  if ((typeof num!='number') && (typeof num!='string'))
    return mkuni.str4code(mkuni.asciizero);
  var str=((typeof num=='string') ? num : num.toString());
  var idx=str.indexOf(mkuni.str4code(mkuni.asciidec));
  if (idx<0)
    idx=str.length;
...
mkfloat.roundpos=function(num,pos,epspos) {
  ...
  var ii=0,ll=stri.length,phony1=0,idx=stri.indexOf(mkuni.str4code(mkuni.asciidec));
  if (idx<0)
    idx=ll;
  if ((idx+pos)>=ll)
    return num;
  ...
  if ((idx+pos)<=0)
    return 0;
  chk.splice(idx,0,mkuni.str4code(mkuni.asciidec));
  if (phony1==1) {
