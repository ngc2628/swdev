...
hold on;
plot3(xbc,ybc,zbc,...
  'linestyle','none','marker','*','markersize',1,'color',[0.5,0.6,0.5]);
%figure(2);
%hold on;
...
x0a=[cl(2).pp(4).xx-2500.0,cl(2).pp(4).yy-0.15,cl(2).pp(4).zz];
figure(1);
hold on;
plot3(x0a(1),x0a(2),x0a(3),...
  'linestyle','none','marker','o','markersize',7,'markerfacecolor',[0.1,0.3,0.5],'color',[0.1,0.3,0.5]);
ststart=[0.5,0.5];
[fail,sta]=Oe04gba(x0a,ststart,nsx,ntx,lamdax,mux,cx,nsy,nty,lamday,muy,cy,nsz,ntz,lamdaz,muz,cz);
if (length(fail)>0)
  printf ("%d, %s",__LINE__,fail);
endif
...
x0b=[cl(3).pp(1).xx,cl(3).pp(1).yy,cl(3).pp(1).zz];
figure(1);
hold on;
...
% ----- calc y4x and x4y in 2d-projection-space (z==0) 
%[fail,parx,pary]=Oe04ab(0.0,1.0,5000.0,spx(1).nn,spx(1).lamda,spx(1).cc);
%parx
%pary
%[fail,pary]=Oe02bb(spy(1).nn,spy(1).lamda,spy(1).cc,parx);
%pary

%[fail,parx,pary]=Oe04ab(0.0,1.0,1.15345661643906,spy(1).nn,spy(1).lamda,spy(1).cc);
%parx
%pary
%[fail,pary]=Oe02bb(spx(1).nn,spx(1).lamda,spx(1).cc,parx);
%pary
% ----- calc y4x and x4y in 2d-projection-space (z==0) 

% ----- calc boundary distance in 2d-projection-space (z==0) 
[fail,parx,pary]=Oe04ab(0.0,1.0,[cl(2).pp(2).xx,cl(2).pp(2).yy],...
  tpx(1).nn,tpx(1).lamda,tpx(1).cc,tpy(1).nn,tpy(1).lamda,tpy(1).cc);
parx
pary
[fail,bdx]=Oe02bb(tpx(1).nn,tpx(1).lamda,tpx(1).cc,parx);
[fail,bdy]=Oe02bb(tpy(1).nn,tpy(1).lamda,tpy(1).cc,parx);
bdx
bdy
sqrt((bdx-cl(2).pp(2).xx)*(bdx-cl(2).pp(2).xx)+(bdy-cl(2).pp(2).yy)*(bdy-cl(2).pp(2).yy))
% ----- calc boundary distance in 2d-projection-space (z==0) 
