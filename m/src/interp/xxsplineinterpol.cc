...
DEFUN_DLD (xxsplineinterpol, args, cntout,"xxsplineinterpol\n")
{
  octave_value_list res;

  int nneval=args(0).int_value();
  ...
  double *xxint=(double*)malloc(nneval*sizeof(double));
  double *yyint=(double*)malloc(nneval*sizeof(double));

  ok=sp.interpol(nneval,xxint,yyint);
  ...
  ndxx.resize1(nneval);
  ndyy.resize1(nneval);
  for (ii=0;ii<nneval;ii++) {
    ndxx.elem(ii)=xxint[ii];
    ndyy.elem(ii)=yyint[ii];
  }
  res(0)=ndxx;
  res(1)=ndyy;
